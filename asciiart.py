...
from nearest_color import ANSITree
...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-method",
        action="store",
        choices=["mean", "lightness", "luminosity"],
        default="lightness",
...
def single_value(array, method="mean"):
    if method == "mean":
        ...
        luminosity = lambda a: (a * np.array([0.21, 0.72, 0.07])).sum()
...
def tree_based_closest_ANSI(color: list, ansi_tree):
    return ansi_tree.nearest_ansi(color)


# Create ANSI color mask from RGB image array
def tree_color_mask(imarray):
    ansi_dict = pickle.load(open("ansi_color_dict.pkl", "rb"))
    ansi_tree = ANSITree(ansi_dict)
    return np.apply_along_axis(
        tree_based_closest_ANSI, 2, imarray, ansi_tree
    )


def brute_color_mask(imarray):
...
def snapshot():
    try:
        ...
        raise Exception(
            "No file specified and imagesnap not available. See 'python asciiart.py --help'"
        )
...
def main():
    ...
    if args.color == True:
        color_array = tree_color_mask(imarray)
